<%@page import="com.cliente.dto.EstudianteDTO"%>
...
<title>Gestión de estudiantes - UTEC</title>
</head>
	...
	<div class="app">
		...
		<main class="contenido">
			...
			<section class="gestionContenido">
				...
				<%
				ArrayList<EstudianteDTO> listaDeEstudiantes = ServiceEstudiante.listarActivoFiltro(filtro);
				...
				<div class="tableContenido">
					<table>
						...
						<tbody>
							<%
							for (EstudianteDTO oEstudiante : listaDeEstudiantes) {
							%>
							<tr>
								<td><%=oEstudiante.getoUsuario().getDocumento()%></td>
								<td><%=oEstudiante.getoUsuario().getNombres()%></td>
								<td><%=oEstudiante.getoUsuario().getApellidos()%></td>
								<td><%=oEstudiante.getoUsuario().getMailPersonal()%></td>
								<td><%=oEstudiante.getoUsuario().getMailInstitucional()%></td>
								<td><%=oEstudiante.getoUsuario().getGenero().getNombre()%></td>
								<td><%=oEstudiante.getoUsuario().getDepartamento().getNombre()%></td>
								<td><%=oEstudiante.getoUsuario().getLocalidad().getNombre()%></td>
								<td><%=oEstudiante.getoUsuario().getItr().getNombre()%></td>
								<td><%=oEstudiante.getoUsuario().getTelefono()%></td>
								<td><%=oEstudiante.getoUsuario().getFechaNacimiento()%></td>
								<td>
									<%
									if (filtro.equals("S")) {
									%>
									<div>
										<form name="editar" action="/Proyecto-PInfra/SvEditarUsuario"
											...
											<input type="hidden" name="cedula"
												value="<%=oEstudiante.getoUsuario().getDocumento()%>">
										</form>
										<form name="eliminar"
											...
											<input type="hidden" name="idUsuario"
												value="<%=oEstudiante.getoUsuario().getIdUsuario()%>">
										</form>
									</div> <%
 } else {
 %>
