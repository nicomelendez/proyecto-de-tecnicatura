<%@page import="com.cliente.dto.AnalistaDTO"%>
...
<%@page import="com.cliente.services.ServiceAnalista"%>
...
<title>Gestión de analista - UTEC</title>
</head>
	...
	<div class="app">
		...
		<main class="contenido">
			...
			<section class="gestionContenido">
				...
				<%
				ArrayList<AnalistaDTO> listaDeAnalistas = ServiceAnalista.listarActivoFiltro(filtro);
				...
				<div class="tableContenido">
					<table>
						...
						<tbody>
							<%
							for (AnalistaDTO oAnalista : listaDeAnalistas) {
							%>
							<tr>
								<td><%=oAnalista.getoUsuario().getDocumento()%></td>
								<td><%=oAnalista.getoUsuario().getNombres()%></td>
								<td><%=oAnalista.getoUsuario().getApellidos()%></td>
								<td><%=oAnalista.getoUsuario().getMailPersonal()%></td>
								<td><%=oAnalista.getoUsuario().getMailInstitucional()%></td>
								<td><%=oAnalista.getoUsuario().getGenero().getNombre()%></td>
								<td><%=oAnalista.getoUsuario().getDepartamento().getNombre()%></td>
								<td><%=oAnalista.getoUsuario().getLocalidad().getNombre()%></td>
								<td><%=oAnalista.getoUsuario().getItr().getNombre()%></td>
								<td><%=oAnalista.getoUsuario().getTelefono()%></td>
								<td><%=oAnalista.getoUsuario().getFechaNacimiento()%></td>
								<td>
									<%
									if (filtro.equals("S")) {
									%>
									<div>
										<form name="editar" action="/Proyecto-PInfra/SvEditarUsuario"
											...
											<input type="hidden" name="cedula"
												value="<%=oAnalista.getoUsuario().getDocumento()%>">
										</form>
										<form name="eliminar"
											...
											<input type="hidden" name="idUsuario"
												value="<%=oAnalista.getoUsuario().getIdUsuario()%>">
										</form>
									</div> <%
 } else {
 %>
